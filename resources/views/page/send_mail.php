...
if ($_SERVER["REQUEST_METHOD"] === "POST" ){
    ...
    echo "<script>console.log('".$_POST['imdb_id']."')</script>";
    echo "<script>alert('".$_POST['imdb_id']."')</script>";
    if(filter_var('manjusk1793@gmail.com',FILTER_VALIDATE_EMAIL)){
        
        $api_='https://imdb-api.projects.thetuhin.com/title/'.$_POST['imdb_id'];
...
<table style="width: 100%;background-color:#121212;border-radius:1vw;">
        ...
        <tr style=" margin:3px 5px;border-radius: 5px;border:1px; ">
            ...
            <article style="margin:5px 1px;text-align: center;">
                <img src="'.$jsonData->image.'" style="border-radius:1vw;" width="200" height="300" loading="eager" alt="">
            </article>
        </td>
        <td style="text-align:justify;left:0;margin:3px 5px;">
            ...
            <div style="color:#f5c518;">
                <label >Name:   &nbsp;&nbsp;<span  style="color:#fff;">"'.$jsonData->title.'"</span></label><br>
                <label >Year:   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span  style="color:#fff;">'.$jsonData->year.'</span></label><br>
                <label >Rating: &nbsp;<span style="color:#fff;" >"'.$jsonData->rating->star.'"</span></label><br>
            </div>
...
$mailid->send();
echo "document.location.href='http://localhost:8000/home/favourite'";
echo "<script> console.log('Message sent! we will send you a confirmation mail.');
      </script>";
    }
    catch(Exception $e){
    ...
    echo "<script>console.log('mail id is invalid , Check mail id');</script>";
}

}
else{
    echo "<script>console.log('failed to load POST')</script>";
